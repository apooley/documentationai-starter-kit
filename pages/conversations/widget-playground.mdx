...
|---|---|
| `/clear` | Fully resets the widget session — wipes the stored contact (name and email), conversation, draft, and session tracking, then restarts name/email collection from scratch. This does not send any message to the AI. |
| `/flag-good` | Marks the current conversation as a good training example |
| `/flag-bad` | Marks the current conversation as a bad training example |
